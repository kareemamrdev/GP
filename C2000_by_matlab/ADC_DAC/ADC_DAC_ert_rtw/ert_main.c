...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) ADC_DAC_M->Timing.clockTick0;
  ADC_DAC_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(ADC_DAC_M->extModeInfo, &rtmGetTFinal(ADC_DAC_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ADC_DAC_M, true);
    }
  }
